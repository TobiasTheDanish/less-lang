...
compiler_T *compiler_new(ast_node_T *program, symbol_table_T *s_table,
...
// assign : (ID | array_element) ASSIGN (value | bin_op | array | array_element
// | prop) ;
void compile_assign(compiler_T *c, ast_node_T *node) {
  ...
  default:
    log_error(
        node->loc, 1,
        "Unexpected symbol type in assignment. Found %s, expected %s or %s.\n",
        symbol_get_type_string(sym->type), symbol_get_type_string(SYM_VAR),
        symbol_get_type_string(SYM_PROP));
  }
}

...
#define MAX_REGS 13
char *regs_64bit[MAX_REGS] = {"rdi", "rsi", "rdx", "rcx", "r8",  "r9", "rax",
                              "r10", "r11", "r12", "r13", "r14", "r15"};
char *all_regs[MAX_REGS * 4] = {
    "dil",  "sil",  "dl",   "cl",   "r8b",  "r9b",  "al",   "r10b", "r11b",
    "r12b", "r13b", "r14b", "r15b", "di",   "si",   "dx",   "cx",   "r8w",
    "r9w",  "ax",   "r10w", "r11w", "r12w", "r13w", "r14w", "r15w", "edi",
    "esi",  "edx",  "ecx",  "r8d",  "r9d",  "eax",  "r10d", "r11d", "r12d",
    "r13d", "r14d", "r15d", "rdi",  "rsi",  "rdx",  "rcx",  "r8",   "r9",
...
void compile_expr(compiler_T *c, lir_node_T node);
...
//     if (prop->node == NULL) {
//       symbol_type_T *prop_type = (symbol_type_T *)symbol_get_prop_type(
//           prop_sym->type, prop->prop->value);
//       char *reg = prop_type->regs[reg_no];
//       size_t offset = symbol_get_prop_offset(prop_sym->type,
//       prop->prop->value);
//
...
//       char str[40];
//       if (!var_sym->is_const) {
//         snprintf(str, 40, "%s, %s [rbp+%zu]\n", reg, var_type->operand,
//                  (8 * (var_sym->index + 1)) + 2);
//       } else if (var_sym->is_const) {
//         size_t index = data_table_get_index(c->data_table,
//         var_sym->const_val); snprintf(str, 40, "%s, %s [const%zu]\n", reg,
//         var_type->operand, index);
//       }
//
//       append_file(c->file, str);
//       return reg;
...
//     default:
//       log_error(
//           node->loc, 1, "Unexpected symbol type, expected %s, found %s.\n",
//           symbol_get_type_string(SYM_VAR),
//           symbol_get_type_string(sym->type));
//     }
//     break;
//   }
//
...
//
//   default:
//     log_error(bin_op->rhs->loc, 1,
//               "Unexpected node in bin_op rhs. Found: %s.\n",
...
//
//   default:
//     log_error(bin_op->lhs->loc, 1,
//               "Unexpected node in bin_op lhs. Found: %s.\n",
//               ast_get_name(bin_op->rhs->type));
//   }
//   return compile_op(c, bin_op->op, reg2, reg1);
// }
//
// // array_element : ID LSQUARE (array_element | IDENT | INTEGER | bin_op |
// prop)
// // RSQUARE ;
// char *compile_array_element(compiler_T *c, ast_node_T *node, size_t reg_no,
//                             symbol_T *parent_sym) {
...
//   size_t offset = 0;
//   if (parent_sym == NULL) {
//     symbol_var_T *arr_sym =
//         (symbol_var_T *)symbol_table_get(c->s_table, arr->ident->value);
//     if (arr_sym == NULL) {
//       log_error(node->loc, 1,
//                 "Uninitialized symbol '%s' used in compile_array_element\n",
//                 arr->ident->value);
//     }
...
//     symbol_prop_T *prop_sym =
//         (symbol_prop_T *)symbol_get_prop(parent->type, arr->ident->value);
//     if (prop_sym == NULL) {
//       log_error(node->loc, 1, "Invalid property '%s' for type '%s'\n",
...
//     NULL); ast_array_element_T *arr2 = (ast_array_element_T *)arr->offset;
//     symbol_var_T *arr_sym =
//         (symbol_var_T *)symbol_table_get(c->s_table, arr2->ident->value);
//
//     char *end =
//         ((symbol_type_T *)arr_sym->elem_type)->regs[(reg_no + 1) % MAX_REGS];
...
//
//   snprintf(str, 50, "    xor %s, %s\n", regs_64bit[reg_no],
//   regs_64bit[reg_no]); append_file(c->file, str); snprintf(str, 50, "    mov
//   %s, %s %s\n", regs_64bit[reg_no],
//            type_sym->operand, mem_location);
//   if (parent_sym != NULL) {
//     append_file(c->file, str);
...
//
//   default:
//     log_error(dump->value->loc, 1,
//               "Unexpected node in dump. Found: %s, expects: %s or %s.\n",
//               ast_get_name(dump->value->type), ast_get_name(AST_BIN_OP),
//               ast_get_name(AST_VALUE));
//   }
//
//   append_file(c->file, "\n    call  _dump\n");
//   c->stack_pointer -= 1;
...
//
//   default:
//     log_error(elze->block->loc, 1,
//               "Unexpected ast type for compile_else. Found: %s\n",
...
// }
//
// // attribute : ID ASSIGN (value | array_element | prop | bin_op | array)
// COMMA? void compile_prop_init(compiler_T *c, ast_node_T *node, symbol_T
// *type,
//                        const char *mem_reg) {
...
//   symbol_type_T *symbol =
//       (symbol_type_T *)symbol_table_get(c->s_table, structure->struct_name);
//
...
//     log_error(node->loc, 1,
//               "Uninitialized symbol '%s' used in compile_assign\n",
//               a->ident->value);
//   }
//
//   if (sym == NULL) {
...
//   } else {
//     log_error(a->value->loc, 1,
//               "Unexpected ast type for rhs of compile_assign. Found: %s\n",
...
//     } else {
//       log_error(node->loc, 1,
//                 "Something is seriously wrong with assignment of properties:
//                 " "symbol: %s\n", sym->name);
//     }
//     break;
//
...
//
//       snprintf(str, 40, "    mov %s [%s], %s\n", var_type->operand, reg2,
//                sized_reg);
//       append_file(c->file, str);
//     } else if (a->lhs->type == AST_VALUE) {
//       symbol_type_T *var_type = (symbol_type_T *)var_sym->type;
//       char *sized_reg =
//           var_type->is_primitive ? match_reg_to_type(var_type->size, reg) :
//           reg;
//       append_file(c->file, "    mov ");
...
//     log_error(
//         node->loc, 1,
//         "Unexpected symbol type in assignment. Found %s, expected %s or
//         %s.\n", symbol_get_type_string(sym->type),
//         symbol_get_type_string(SYM_VAR), symbol_get_type_string(SYM_PROP));
//   }
// }
//
...
//
//   default:
//     log_error(node->loc, 1, "Invalid node type in compile_sys_arg. Found:
//     %s\n",
...
//               "Unexpected symbol type found when compiling function params. "
//               "Found %s, expected %s\n",
//               symbol_get_type_string(sym->type),
//               symbol_get_type_string(SYM_VAR));
//   }
...
//            (8 * (param_sym->index + 1)) + 2, param_type->regs[param_index]);
//   log_debug(c->debug, "\t%s\n", str);
//   append_file(c->file, str);
// }
//
...
//   case AST_ATTRIBUTE:
//     log_error(node->loc, 1,
//               "Invalid node in function call argument. Found %s\n",
//               ast_get_name(node->type));
//   }
// }
//
...
//   symbol_func_T *func_sym = (symbol_func_T *)symbol_table_get(c->s_table,
//   name);
//
...
//         log_error(NULL, 1, "Compile array_expr not implemented for move
...
//         } else {
//                 log_error(expr->rhs->loc, 1, "Unexpected node type as rhs in
...
void compile_mod(compiler_T *c, lir_node_T node) {
  if (node.count > 2) {
    log_error(NULL, 1,
              "Too many children for mod operation, expects 2 but found %d\n",
              node.count);
  } else if (node.count < 2) {
    log_error(NULL, 1,
              "Too few children for mod operation, expects 2 but found %d\n",
              node.count);
  }

  if (node.children[0].tag == LIR_VALUE && node.children[1].tag == LIR_VALUE) {
    append_file(c->file, "    mov rax, %s\n", node.children[0].value);
    append_file(c->file, "    cqo\n");
    append_file(c->file, "    mov rdi, %s\n", node.children[1].value);
    append_file(c->file, "    idiv rdi\n");
    append_file(c->file, "    push rdx\n");
  } else if (node.children[0].tag == LIR_VALUE) {
    ...
    append_file(c->file, "    idiv rdi\n");
    append_file(c->file, "    push rdx\n");
  } else if (node.children[1].tag == LIR_VALUE) {
    compile_expr(c, node.children[0]);
    append_file(c->file, "    pop rax\n");
    append_file(c->file, "    cqo\n");
    append_file(c->file, "    mov rdi, %s\n", node.children[1].value);
    append_file(c->file, "    idiv rdi\n");
...
void compile_div(compiler_T *c, lir_node_T node) {
  if (node.count > 2) {
    log_error(NULL, 1,
              "Too many children for div operation, expects 2 but found %d\n",
              node.count);
  } else if (node.count < 2) {
    log_error(NULL, 1,
              "Too few children for div operation, expects 2 but found %d\n",
              node.count);
  }
...
void compile_mul(compiler_T *c, lir_node_T node) {
  if (node.count > 2) {
    log_error(NULL, 1,
              "Too many children for mul operation, expects 2 but found %d\n",
              node.count);
  } else if (node.count < 2) {
    log_error(NULL, 1,
              "Too few children for mul operation, expects 2 but found %d\n",
              node.count);
  }

  if (node.children[0].tag == LIR_VALUE && node.children[1].tag == LIR_VALUE) {
    append_file(c->file, "    mov rdi, %s\n", node.children[0].value);
    append_file(c->file, "    imul rdi, %s\n", node.children[1].value);
    append_file(c->file, "    push rdi\n");
  } else if (node.children[0].tag == LIR_VALUE) {
    compile_expr(c, node.children[1]);
    append_file(c->file, "    mov rdi, %s\n", node.children[0].value);
    append_file(c->file, "    pop rax\n");
    append_file(c->file, "    imul rdi, rax\n");
    append_file(c->file, "    push rdi\n");
  } else if (node.children[1].tag == LIR_VALUE) {
    compile_expr(c, node.children[0]);
    append_file(c->file, "    pop rdi\n");
    ...
    append_file(c->file, "    pop rdi\n");
    append_file(c->file, "    pop rsi\n");
    append_file(c->file, "    imul rdi, rsi\n");
    append_file(c->file, "    push rdi\n");
  }
}

void compile_sub(compiler_T *c, lir_node_T node) {
  if (node.count > 2) {
    log_error(NULL, 1,
              "Too many children for sub operation, expects 2 but found %d\n",
              node.count);
  } else if (node.count < 2) {
    log_error(NULL, 1,
              "Too few children for sub operation, expects 2 but found %d\n",
              node.count);
  }

  if (node.children[0].tag == LIR_VALUE && node.children[1].tag == LIR_VALUE) {
    append_file(c->file, "    mov rdi, %s\n", node.children[0].value);
    append_file(c->file, "    sub rdi, %s\n", node.children[1].value);
    append_file(c->file, "    push rdi\n");
  } else if (node.children[0].tag == LIR_VALUE) {
    compile_expr(c, node.children[1]);
    append_file(c->file, "    mov rdi, %s\n", node.children[0].value);
    append_file(c->file, "    pop rax\n");
    append_file(c->file, "    sub rdi, rax\n");
    append_file(c->file, "    push rdi\n");
  } else if (node.children[1].tag == LIR_VALUE) {
    compile_expr(c, node.children[0]);
    append_file(c->file, "    pop rdi\n");
    ...
    append_file(c->file, "    pop rdi\n");
    append_file(c->file, "    pop rsi\n");
    append_file(c->file, "    sub rdi, rsi\n");
    append_file(c->file, "    push rdi\n");
  }
}

void compile_add(compiler_T *c, lir_node_T node) {
  if (node.count > 2) {
    log_error(NULL, 1,
              "Too many children for add operation, expects 2 but found %d\n",
              node.count);
  } else if (node.count < 2) {
    log_error(NULL, 1,
              "Too few children for add operation, expects 2 but found %d\n",
              node.count);
  }

  if (node.children[0].tag == LIR_VALUE && node.children[1].tag == LIR_VALUE) {
    append_file(c->file, "    mov rdi, %s\n", node.children[0].value);
    append_file(c->file, "    add rdi, %s\n", node.children[1].value);
    append_file(c->file, "    push rdi\n");
  } else if (node.children[0].tag == LIR_VALUE) {
    compile_expr(c, node.children[1]);
    append_file(c->file, "    mov rdi, %s\n", node.children[0].value);
    append_file(c->file, "    pop rax\n");
    append_file(c->file, "    add rdi, rax\n");
    append_file(c->file, "    push rdi\n");
  } else if (node.children[1].tag == LIR_VALUE) {
    compile_expr(c, node.children[0]);
    append_file(c->file, "    pop rdi\n");
    ...
    append_file(c->file, "    pop rdi\n");
    append_file(c->file, "    pop rsi\n");
    append_file(c->file, "    add rdi, rsi\n");
    append_file(c->file, "    push rdi\n");
  }
}

void compile_func_param(compiler_T *c, symbol_T *sym, size_t param_index) {
  if (sym->tag != SYM_VAR) {
    log_error(NULL, 1,
              "Unexpected symbol type found when compiling function params. "
              "Found %s, expected %s\n",
              symbol_get_type_string(sym->tag),
              symbol_get_type_string(SYM_VAR));
  }

  symbol_var_T param_sym = sym->var;
  symbol_T *param_type = param_sym.type;
  log_debug(c->debug, "param type name: %s\n", param_type->name);
  append_file(c->file, "    mov %s [rbp+%zu], %s\n", param_type->type.operand,
              (8 * (param_sym.index + 1)) + 2,
              param_type->type.regs[param_index]);
}

void compile_label(compiler_T *c, lir_node_T node) {
  append_file(c->file, "_");
  append_file(c->file, node.value);
  append_file(c->file, ":\n");
  append_file(c->file, "    push rbp\n");
  append_file(c->file, "    mov rbp, rsp\n");
  if (strcmp(node.value, "main") == 0) {
    append_file(c->file, "    sub rsp, %u\n",
                (c->s_table->child_count + 1) * 8);
  }

  symbol_T *func_symbol = symbol_table_get(c->s_table, node.value);

  if (func_symbol->tag != SYM_FUNC) {
    log_error(NULL, 1, "Label ir node '%s' is not a function\n", node.value);
  }

  c->s_table = func_symbol->func.scope;
  size_t param_count = 0;
  symbol_T *params = symbol_table_get_params(c->s_table, &param_count);

  for (unsigned char i = param_count; i > 0; i--) {
    compile_func_param(c, &params[i - 1], i - 1);
  }
...
void compile_dump(compiler_T *c, lir_node_T node) {
  log_debug(c->debug, "compile dump\n");
  append_file(c->file, "    ; -- dump --\n");
...
// expr : syscall SEMI | if | while | var_decl | const_decl | array_expr SEMI
// assign SEMI | bin_op SEMI | dump SEMI | func_decl | func_call SEMI ;
void compile_expr(compiler_T *c, lir_node_T node) {
  switch (node.tag) {
  case LIR_LABEL:
    return compile_label(c, node);
  case LIR_DUMP:
    return compile_dump(c, node);
  case LIR_ADD:
    return compile_add(c, node);
  case LIR_SUB:
    return compile_sub(c, node);
  case LIR_MUL:
    return compile_mul(c, node);
  case LIR_DIV:
    return compile_div(c, node);
  case LIR_MOD:
    return compile_mod(c, node);
  case LIR_CALL:
  case LIR_VALUE:
  case LIR_IDENT:
  case LIR_PUSH:
  case LIR_LOAD:
  case LIR_STORE:
    log_todo("compile %s not implemented yet\n", lir_tag_name(&node));
    break;
  case LIR_PROGRAM:
  case LIR_TAG_COUNT:
    log_error(NULL, 1, "Unexpected node in expression '%s'.\n",
              lir_tag_name(&node));
  }
}

void compile_program(compiler_T *c, lir_node_T *node) {
  if (node->tag == LIR_PROGRAM) {

    for (size_t i = 0; i < node->count; i++) {
      log_debug(c->debug, "Program expr #%u type: %s\n", i,
                lir_tag_name(&node->children[i]));
      compile_expr(c, node->children[i]);
    }
  } else {
    log_error(
        NULL, 1,
        "Unexpected node to start program.\n Found: %s, expects: Program.\n",
        lir_tag_name(node));
  }
}
...
void compile(compiler_T *c) {
  ...
  // append_file(c->file, "    mov     rdi, 30\n");
  // append_file(c->file, "    call     alloc\n");

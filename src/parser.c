...
void type_annotation(parser_T *parser, symbol_T *type_sym,
                     symbol_T *elem_type) {
...
ast_node_T *prop(parser_T *parser) {
  ...
  if (!symbol_is_prop(v->type, prop_token->value)) {
    ...
    log_error(prop_token->loc, 1,
              "Unknown property '%s' for identifier '%s' of type '%s'\n",
...
ast_node_T *value(parser_T *parser) {
  ...
  case T_POINTER:
    if (symbol_table_contains(parser->s_table, token->value)) {
      symbol_var_T *var =
          (symbol_var_T *)symbol_table_get(parser->s_table, token->value);
  ...
  case T_INTEGER: {
    // printf("<%s, %s>\n", token->value, "int");
    char *type;
    if (atoi(token->value) < 255)
      type = "i8";
    else if (atoi(token->value) < 65535)
      type = "i16";
    else
      type = "i32";
...
// bin_op: (value | prop | array_element) op (bin_op | value | prop |
// array_element) ;
ast_node_T *bin_op(parser_T *parser, ast_node_T *lhs) {
  ...
  if (lhs->type == AST_PROP) {
    ...
    symbol_var_T *prop_sym =
        (symbol_var_T *)symbol_table_get(parser->s_table, v->parent_sym->name);
    lhs_type = (symbol_type_T *)prop_sym->elem_type;
  } else if (lhs->type == AST_ARRAY_ELEMENT) {
    ast_array_element_T *v = (ast_array_element_T *)lhs;
    symbol_var_T *elem_sym =
        (symbol_var_T *)symbol_table_get(parser->s_table, v->ident->value);
  ...
  if ((current->type == T_INTEGER || current->type == T_IDENT) &&
      next->type == T_LSQUARE) {
    ...
    /*
            ...
            log_error(current->loc, 1, "Mismatched types for bin op. Found: '%s'
    ...
    */
    ...
    log_error(current->loc, 1,
              "Invalid token type for rhs in bin_op. Found: %s, expected an "
              "identifier, value or bin_op\n",
...
ast_node_T *attribute(parser_T *parser, symbol_type_T *type) {
  ...
  symbol_T *val_sym =
      symbol_table_get(parser->s_table, parser->tokens[parser->t_index]->value);
  ast_node_T *val;
  symbol_T *val_type;
  token_T *next = parser->tokens[(parser->t_index + 1) % parser->t_count];
  if (next->type == T_DOT) {
  ...
  } else if (val_sym != NULL && val_sym->type == SYM_VAR_TYPE &&
             next->type == T_LSQUARE) {
    val = array(parser);
    ast_array_T *arr = (ast_array_T *)val;
    val_type = arr->type;
  } else if (val_sym != NULL && val_sym->type == SYM_VAR &&
             next->type == T_LSQUARE) {
    val = array_element(parser);
    symbol_var_T *var = (symbol_var_T *)val_sym;
  ...
  if (val_type == NULL) {
    log_error(ident->loc, 1,
              "Value in assignment didnt have a type. Expected %s.\n",
              prop_type->name);
  }
  /*
  if (val_type == NULL || strcmp(val_type->name, prop_type->name) != 0) {
          log_error(ident->loc, 1, "Mismatched types in struct initialization.
  Found %s and expected %s.\n", val_type->name, prop_type->name);
  }
  */
...
ast_node_T *struct_init(parser_T *parser) {
  ...
  if (id->type != SYM_VAR_TYPE) {
    log_error(
        ident->loc, 1,
        "Attempt at initializing a variable of type '%s'. '%s' is not a type.",
...
ast_node_T *assign(parser_T *parser, ast_node_T *lhs) {
  ...
  case AST_PROP: {
    ...
    parent_sym = (symbol_T *)parent;
    parent_sym_type = parent->type;
  } break;

  default:
    log_error(lhs->loc, 1,
              "Invalid left hand side of assignment. Cannot assign to %s.\n",
  ...
  if (symbol_table_contains(parser->s_table, ident->value)) {
    ...
    if (symbol->is_const) {
      ...
      exit(1);
               ...
               lhs->type != AST_ARRAY_ELEMENT) {
      log_error(ident->loc, -1, "Cannot reassign immutable variable '%s'\n",
                ident->value);
      location_T *loc = symbol->base.loc;
      log_info(
          "Consider adding 'mut' to declaration of '%s' here: '%s:%d:%d'\n",
          ident->value, loc->filePath, loc->row, loc->col);
      exit(1);
    }
  } else if (parent_sym != NULL && parent_sym_type != NULL && ident != NULL &&
             symbol_is_prop(parent_sym_type, ident->value)) {
    symbol = (symbol_var_T *)parent_sym;
    if (symbol->is_const) {
      log_error(ident->loc, -1,
                "Cannot assign to property of constant identifier '%s'\n",
                parent_sym->name);
      location_T *loc = symbol->base.loc;
      log_info("Constant identifier '%s' defined here: '%s:%d:%d'\n",
               parent_sym->name, loc->filePath, loc->row, loc->col);
      exit(1);
    } else if (!symbol->is_mut && symbol->is_assigned &&
               lhs->type != AST_ARRAY_ELEMENT) {
      log_error(ident->loc, -1,
                "Cannot assign to property of immutable variable '%s'\n",
                parent_sym->name);
      location_T *loc = symbol->base.loc;
      log_info(
          "Consider adding 'mut' to declaration of '%s' here: '%s:%d:%d'\n",
  ...
  if (current->type == T_SYSCALL) {
    ...
    if (!f_sym->ret_type) {
      log_error(f->ident->loc, 1,
                "Cannot assign with function '%s' that returns void.\n",
                f->ident->value);
    }
    if (symbol && symbol->type == NULL && !symbol->is_assigned) {
...
ast_node_T *const_decl(parser_T *parser) {
  ...
  if (!symbol_table_contains(parser->s_table, ident->value)) {
    ...
    symbol_var_T *s = (symbol_var_T *)symbol_new_var(
        ident->value, ident->loc, val->type_sym, 0, 0, 1, val->t->value);
...
token_T *func_param(parser_T *parser, symbol_func_T *func) {
  ...
  if (type_sym != NULL) {
    ...
    case SYM_VAR_TYPE: {
      symbol_T *var_sym =
          symbol_new_var(param->value, param->loc, type_sym, is_mut, 1, 0, 0);
    ...
    default:
      log_error(
          type_sym->loc, 1,
          "Unknown type annotation. Found symbol '%s', symbol type: '%s'.\n",
          type_sym->name, symbol_get_type_string(type_sym->type));
    }
  } else {
    log_error(param->loc, 1,
              "Could not extract type annotation for parameter %s\n",
              param->value);
  }
...
// func_decl : FUNC ID LPAREN (func_param (COMMA func_param)*)? RPAREN (COLON
// ID)? block ;
ast_node_T *func_decl(parser_T *parser) {
  ...
  symbol_table_T *func_table = symbol_table_new(
      ident->value, parser->s_table->level + 1, parser->s_table);
...
ast_node_T *arg(parser_T *parser, symbol_var_T *param) {
  ...
  if (current->type == T_IDENT || current->type == T_POINTER ||
    ...
    if (next->type == T_LSQUARE) {
      ...
      symbol_var_T* arr = (symbol_var_T*)symbol_table_get(parser->s_table,
      val->ident->value); if (strcmp(arr->elem_type->name, param->type->name) !=
      0) { log_error(current->loc, 1, "Mismatched argument types in function
      call, found: %s, expected: %s.\n", arr->elem_type->name,
      param->type->name);
      }
      */
    } else if (next->type == T_DOT) {
      ...
      if (strcmp(p_type->name, param->type->name) != 0) {
              log_error(current->loc, 1, "Mismatched argument types in function
...
ast_node_T *func_call(parser_T *parser, ast_node_T *arg1) {
  ...
  if (func_sym == NULL || func_sym->type != SYM_FUNC) {
    log_error(ident->loc, 1,
              "Unknown function '%s' is attempted to be called.\n",
  ...
  while (parser->tokens[parser->t_index]->type != T_RPAREN) {
    if (count >= func->param_count) {
      log_error(parser->tokens[parser->t_index]->loc, 1,
                "Too many arguments for function %s. Expected %u.\n",
  ...
  if (count < func->param_count) {
    log_error(parser->tokens[parser->t_index]->loc, 1,
              "Too few arguments for function %s. Found %u, expected %u.\n",
              func->base.name, count, func->param_count);
  }
  consume(parser, T_RPAREN);
...
/*
// array_expr : array_element ( SEMI | ((ASSIGN | op) (array_expr | value |
bin_op | array))) ; ast_node_T* array_expr(parser_T* parser) {
        ...
        if (token_is_op(next)) {
                ...
                symbol_T* s = symbol_table_get(parser->s_table,
parser->tokens[parser->t_index]->value); if (s->type == SYM_VAR_TYPE) { rhs =
array(parser); } else { rhs = array_expr(parser);
                }
        } else {
                rhs = value(parser);
        }
...
void struct_decl(parser_T *parser) {
  ...
  symbol_T *type = symbol_new_type(ident->value, ident->loc, 0, struct_size,
                                   props, prop_count);

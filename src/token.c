...
char *token_get_name(token_E type) { return token_names[type]; }

bool token_is_op(token_T *token) {
  return token->type == T_PLUS || token->type == T_MINUS ||
         token->type == T_MULTIPLY || token->type == T_DIVIDE ||
         token->type == T_MODULUS;
}

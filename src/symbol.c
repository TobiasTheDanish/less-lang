...
symbol_T *symbol_upgrade_type(symbol_T *a, symbol_T *b) {
  if (a->tag != SYM_VAR_TYPE || b->tag != SYM_VAR_TYPE) {
    log_error(&((location_T){.col = 0, .row = 0, .filePath = ""}), 1,
              "[INTERNAL]: Tried to upgrade type of non type symbols\n");
  }

  if (a->type.type_cat > b->type.type_cat) {
    return a;
  } else {
    return b;
  }
...
symbol_T symbol_new_type(char *name, location_T *loc,
  ...
  s.scope = symbol_table_new(name, 0, NULL);
...
symbol_T symbol_new_var(char *name, location_T *loc, symbol_T type,
  ...
  var.type = malloc(sizeof(symbol_T));
  memcpy(var.type, &type, sizeof(symbol_T));
...
bool symbol_cmp(symbol_T *a, symbol_T *b) {
  ...
  if (a->tag == SYM_VAR_TYPE) {
    if (strcmp(a->name, b->name) != 0 &&
        !symbol_can_upgrade_type(a->type.type_cat, b->type.type_cat)) {
      return false;
    }

    if (strcmp(a->name, "array") == 0) {
      return symbol_cmp(a->type.underlying_type, b->type.underlying_type);
    }
...
char *symbol_to_string(symbol_T *symbol) {
  if (symbol == NULL) {
    log_warning("Attempted to print symbol that was NULL, skipping.\n");
    return "";
  }
  ...
  case SYM_VAR_TYPE: {
    sprintf(s, "<type:");
    if (symbol->type.is_comptime) {
      sprintf(s, "%s comptime ", s);
    }
    sprintf(s, "%s%s", s, symbol->name);
    if (symbol->type.underlying_type != NULL) {
      sprintf(s, "%s of %s", s, symbol_to_string(symbol->type.underlying_type));
    }
    sprintf(s, "%s>", s);
  } break;

  case SYM_VAR: {
    sprintf(s, "<var:%s, type:%s>", symbol->name,
            symbol_to_string(symbol->var.type));
    // sprintf(s, "<%s:%s:%zu>", var->base.name, var->type->name var->index);
  } break;

  case SYM_FUNC: {
    sprintf(s, "<func: %s, returns: %s>", symbol->name,
            symbol_to_string(symbol->func.ret_type));
  } break;
  }
  return s;
}
